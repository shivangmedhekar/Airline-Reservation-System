...
$sql = "select * from flight where source = '".$_GET['source']."' AND destination='".$_GET['destination']."' AND date_of_flight='".$_GET['date_of_flight']."'";
...
<nav class="navbar navbar-default" style="background-color: darkslateblue">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <h1 style='background-color: darkslateblue;color:white'>Flights Available</h1>
        </div>
...
<div style="background-color: whitesmoke;color:black;height:20px;"></div>
<?php
if($result){
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            if ($row['no_of_seats_available'] == 0)
                echo '<td>Full</td>';
            else
                echo '<td style="width:10%"><input type="radio" name="flight_select" value="'.$row["f_id"].'" checked></td>';
            echo '</tr>';
